...
1. **Submit.** `POST /migrations` with the envelope. Bipa stores it and answers `201` with the current representation; a two-contact conflict is created directly as a terminal denial.
2. **Poll.** `GET /migrations/{id}` returns the state, the stage, the action you should take, and a `user_url` when the person has something to do.
...
Files must be JPEG or PNG, up to 10 MB, and the URLs must stay valid for at least 24 hours. Bipa copies them asynchronously from the public internet, so the URL host must be publicly resolvable and reachable: a hostname that only resolves inside your network, or that points at a private address, is treated as a file that cannot be used. Such a file does not fail the submission; the origination moves to `document_recapture_required` and the person recaptures it inside Bipa.
