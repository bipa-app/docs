...
3. **Correct contacts, if asked.** When both imported contacts belong to other Bipa accounts, the stage is `contact_resolution_required` and `PATCH /v1/originations/{id}/contacts` is the only write allowed.
4. **Read the terminal state.** `settled`, `denied`, or `expired`. Act on your own books and stop polling.
...
| `email`, `phone` | A valid email, lowercased; a valid E.164 phone. |
...
| `pending` | `contact_resolution_required` | `correct_contacts` | Both contacts collide with other accounts. Fix one or both with the contacts endpoint. No `user_url`. |
...
```mermaid
stateDiagram-v2
    validating --> contact_resolution_required
    contact_resolution_required --> validating
...
Every entry into a pending stage starts a 30-day deadline, and `awaiting_activation` starts a 30-day activation window. Always use `deadline_at` from the latest representation; do not compute it locally. A technical retry does not restart a clock; a contact correction does. Expiry with a reserved balance returns the balance to your account before the state becomes `expired`.
...
| `409` | `contact_update_not_allowed` | Contact correction outside `contact_resolution_required`. |
| `413` | | Body over 4 MB. |
