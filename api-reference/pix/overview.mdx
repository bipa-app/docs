...
sequenceDiagram
    ...
    Note over Bipa: Event created (pix_inflow)
    App->>Bipa: Poll Events API
    Bipa-->>App: pix_inflow event (credited)

    Note over App,Bipa: Cash-out Flow
    App->>Bipa: Create payout
    Bipa->>Pix: Send Pix
    Pix-->>Bipa: Confirmation
    Bipa-->>App: Payout response (completed)
```
...
Use the [Events API](/api-reference/partner/events/overview) to track Pix transaction lifecycle changes. Events are generated for:

| Event kind | Log types | Description |
|------------|-----------|-------------|
| `pix_inflow` | `created`, `credited`, `failed` | Incoming payment lifecycle |

Poll for undelivered events regularly and mark them as delivered after processing.
