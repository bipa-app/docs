...
sequenceDiagram
    ...
    Note over Bipa: Transfer status: processing

    Note over Chain: Confirmations...

    Chain->>Bipa: Confirmations complete
    Note over Bipa: Transfer status: confirmed
    Note over Bipa: Balance credited
    App->>Bipa: GET /transfers/{id}
    Bipa-->>App: Transfer (confirmed)
```
...
sequenceDiagram
    ...
    Note over Bipa: Event created (usdt_withdrawal: sent)

    Note over Chain: Transaction confirms

    Chain-->>Bipa: Confirmed
    Note over Bipa: Event created (usdt_withdrawal: settled)
    App->>Bipa: Poll Events API
    Bipa-->>App: usdt_withdrawal event (settled)
```
...
Use the [Events API](/api-reference/partner/events/overview) to track transfer lifecycle changes. For USDT withdrawals, events are generated with the following log types:
